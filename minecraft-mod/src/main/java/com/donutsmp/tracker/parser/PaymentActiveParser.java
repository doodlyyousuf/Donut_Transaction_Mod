...
/**
 * Active-voice player payments, observed in chat:
 *   "Ryuu_3103 paid you $ 200M"        local received
 *   "You paid True_Horror $ 100K"      local sent
 *   "True_Horror paid lucasdegamerboy $ 200K"   observed (payer owns it)
 *
 * Dotted names such as ".RLEIO4566" are preserved verbatim (§10/§41): the
 * leading dot is part of how the server prints that account.
 */
public class PaymentActiveParser implements MessageParser {
    private static final Pattern P = Pattern.compile(
            "^(?<payer>You|[A-Za-z0-9_.]{1,16})\\s+paid\\s+"
                    + "(?<payee>You|[A-Za-z0-9_.]{1,16})\\s+\\$\\s*"
                    + "(?<price>[\\d.,]+\\s*[KkMmBbTt]?)\\s*$",
    ...
    public Optional<TransactionRecord> tryParse(String msg, ParserContext ctx) {
        ...
        String payeeRaw = m.group("payee");
        String payer = payerRaw.equalsIgnoreCase("You") ? local : payerRaw;
        String payee = payeeRaw.equalsIgnoreCase("You") ? local : payeeRaw;
        boolean payerLocal = payer.equalsIgnoreCase(local);
        boolean payeeLocal = payee.equalsIgnoreCase(local);

        TransactionRecord t = new TransactionRecord();
        t.source = "CHAT";
        t.status = "COMPLETED";
        t.observed_by = local;
        t.buyer_username = payer;        // the payer sent the money
        t.recipient_username = payee;    // the payee received it
        t.seller_username = null;        // §41: not a sale
        t.item_name = null;
        t.quantity = null;
        t.total_price = MoneyUtil.parse(m.group("price"));

        if (payerLocal && !payeeLocal) {
